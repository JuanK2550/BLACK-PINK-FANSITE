...
/**
 * Último álbum: la ficha del lanzamiento más reciente con su lista de
 * canciones.
 *
 * Son DOS llamadas encadenadas (el listado para saber cuál es el último y el
 * detalle para su tracklist) porque no hay un endpoint "el más reciente". Las
 * dos respuestas están cacheadas en el gateway y revalidadas una hora en Next,
 * así que en la práctica esto no llega a la red casi nunca.
 */
export async function LatestAlbumSection({ locale }: { locale: Locale }) {
  ...
  try {
    ...
    // Sin álbumes publicados no es un error: es una sección vacía.
    if (!latest) {
      return (
      ...
      <div className="mt-block grid gap-8 lg:grid-cols-12 lg:gap-12">
        <div className="lg:col-span-5">
          <AlbumCover
            ...
            /* Media pantalla en movil, cinco de doce columnas en escritorio. */
