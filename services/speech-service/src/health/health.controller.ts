 ...
 * del prefijo global /api. El resultado seria /v1/health, y los healthcheck
 * del Dockerfile y del compose (que llaman a /health) empezarian a devolver
 * 404 sin que nadie se entere hasta que un contenedor se reinicia en bucle.
 *
 * Este servicio se estreno con el fallo puesto, igual que chatbot-service: sin
 * `configureService` no se notaba, y al conectarlo en esta fase habria
 * aparecido.
 */
@ApiTags('health')
