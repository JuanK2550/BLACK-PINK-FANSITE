 ...
 *
 * El titulo, el orden y quien canta de invitada son AFIRMACIONES, y viven aqui
 * junto a su obra. El `spotifyId` y la duracion son HALLAZGOS de un script y
 * viven en `spotify-ids.json` (seccion `soloTracks`, clave `slug#n`), igual que
 * los de las pistas del grupo: mezclarlos borraria la distincion entre lo que
 * dice una persona y lo que encontro un programa.
 */
export interface SoloTrackSeed {
  title: string;
  trackNumber: number;
  isTitleTrack?: boolean;
  /** Invitadas tal como las acredita el lanzamiento: «Dua Lipa». */
  featuring?: string;
  verified: boolean;
}
...
export interface SoloWorkSeed {
  ...
  /**
   * La lista de canciones del lanzamiento. Opcional solo para que una obra se
   * pueda dar de alta antes de tener su lista contrastada: sin ella, la ficha
   * ensena la obra y su reproductor principal, como antes de la Fase 14.
   */
  tracks?: SoloTrackSeed[];
}

/**
 * Trabajo en solitario de cada integrante.
 *
 * Todas las entradas van verificadas, y el campo `source` de cada una dice
 * contra que se contrasto y cuando. Incluye las colaboraciones de ANTES del
 * debut (2012-2013), cuando YG las acreditaba como «YG New Girl Group»: son
 * canciones publicadas y cantadas por ellas, y la descripcion lo explica.
 *
 * FECHAS EN KST. Un lanzamiento no ocurre en un instante igual para todos:
 * ROCKSTAR salio el 27 de junio de 2024 a las 20:00 ET, que en Corea ya era el
 * 28 a las 09:00. El proyecto adopta la hora coreana, asi que aqui figura el
 * 28. Sin un criterio fijo, el catalogo mezclaria husos y dos fechas
 * igualmente "correctas" se contradirian entre paginas.
 */
export const SOLO_WORKS: SoloWorkSeed[] = [
  ...
  },
  {
    ...
    title: 'Without You',
    type: 'COLLABORATION',
    // Fecha FÍSICA del EP; la digital fue el 15-09-2012 (ver `source`).
  ...
  },
  {
    ...
    title: 'Special',
    type: 'COLLABORATION',
    // Primera publicación: la parte 1 de First Love, solo digital (ver `source`).
  ...
  },
  {
    ...
    title: 'Black',
    type: 'COLLABORATION',
    // El sencillo salió con la parte 1, solo digital (ver `source`).
