...
/**
 * ============================================================================
 * SEED DE content-service
 * ============================================================================
 * Se ejecuta con `pnpm db:seed` (o `prisma db seed`, o automaticamente al
 * final de `prisma migrate reset`).
 *
 * ES IDEMPOTENTE: se puede volver a ejecutar sobre una base con datos sin
 * duplicar nada. Todo entra por `upsert`, y las filas que no tienen una clave
 * natural (curiosidades, hitos, preguntas) usan un id derivado de su propio
 * contenido, de forma que la misma frase produce siempre la misma fila.
 *
 * VALIDA ANTES DE ESCRIBIR. Si una pregunta apunta a una respuesta que no
 * existe, o a una traduccion con distinto numero de opciones, el seed aborta
 * sin tocar la base de datos en lugar de dejarla en un estado incoherente.
 * ============================================================================
 */
...
/** Id estable derivado del contenido: la misma frase da siempre la misma fila. */
function stableId(...parts: string[]): string {
  // El separador es NUL, escrito como escape a proposito. Antes era el byte
  // literal, invisible al leer el fichero -parecia un espacio- y suficiente
  // para que grep tratase este seed como binario. Cambiar el separador
  // cambiaria TODOS los ids y duplicaria el catalogo entero, asi que se
  // conserva el caracter y solo se hace legible.
  return createHash('sha256').update(parts.join('\u0000')).digest('hex').slice(0, 25);
}

/* ==========================================================================
 * VALIDACION PREVIA
 * Se comprueba todo antes de abrir una sola transaccion.
 * ======================================================================= */
  ...
  for (const member of MEMBERS) {
    const row = await prisma.member.upsert({
      ...
      create: {
        ...
        // La foto y su atribucion viajan juntas: la licencia CC BY exige el
        // credito, y separarlo del dato es perderlo en el proximo cambio.
    ...
    for (const locale of LOCALES) {
      const payload = {
        ...
        // El alt va por idioma porque es texto que alguien LEE con un lector
        // de pantalla, y el sitio habla tres.
...
async function seedAwards(): Promise<void> {
  for (const award of AWARDS) {
    ...
    for (const locale of LOCALES) {
      const payload = {
        // El nombre de un premio es un nombre propio y no se traduce: sin
        // traduccion, el servicio cae al original.
...
/* ==========================================================================
 * LO RESUELTO CONTRA SPOTIFY
 *
 * Identificadores y portadas NO se escriben a mano en los ficheros de semilla:
 * los resuelven `infra/scripts/spotify-ids.mjs` y `spotify-covers.mjs` contra
 * la API, y su resultado se versiona en dos volcados JSON que se leen aqui.
 *
 * POR QUE NO ESTAN EN `albums.ts` NI EN `solo-works.ts`: esos ficheros son
 * AFIRMACIONES DE UNA PERSONA, con su `source` y su `verified`. Un `spotifyId`
 * es un HALLAZGO DE UN SCRIPT. Mezclarlos haria imposible distinguir lo uno de
 * lo otro, y esa distincion la usa este proyecto constantemente.
 *
 * POR QUE SE LEEN Y NO SE REGENERAN: relanzar los scripts al sembrar exigiria
 * red y credenciales en todos los entornos, seria no determinista -Spotify
 * cambia- y, sobre todo, PERDERIA LAS DECISIONES MANUAL: al llegar el campo
 * vacio, el script deja de ver la marca y vuelve a resolver por puntuacion.
 *
 * EL VOLCADO MANDA. Si la base y el fichero discrepan, gana el fichero: es el
 * registro. `--check` avisa de la deriva y CI la convierte en un fallo.
 * ======================================================================= */
...
/**
 * Lee un volcado. Su ausencia NO es un error.
 *
 * Una base sembrada sin ellos es una base valida -sin reproductores y con el
 * marco de relleno en vez de portada-, que es exactamente como estuvo el sitio
 * durante varias fases. Fallar aqui convertiria un fichero opcional en un
 * requisito, y rompería el arranque de quien clona el repositorio antes de
 * tener credenciales de Spotify.
 */
...
/**
 * Aplica los identificadores y las portadas ya resueltos.
 *
 * Va DESPUES de sembrar albumes y obras: necesita que las filas existan para
 * localizarlas por su clave de negocio (`born-pink#3`, `rose-r`), que es la
 * misma que usa `--set` en los scripts.
 *
 * UNA ENTRADA QUE NO CASA CON NINGUNA FILA SE AVISA, no se ignora. El caso real
 * que esto detecta es un slug renombrado en `albums.ts` mientras el volcado
 * sigue nombrando el viejo: sin el aviso, esa portada desapareceria en silencio
 * y nadie lo notaria hasta ver el hueco en la pagina.
 */
async function seedResolved(): Promise<{ ids: number; covers: number; orphans: string[] }> {
  ...
  // Las canciones de las obras en solitario, con la misma clave `slug#n`.
...
async function main(): Promise<void> {
  ...
  // Al final: necesita las filas ya creadas para localizarlas por su clave.
