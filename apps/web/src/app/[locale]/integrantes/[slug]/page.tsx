...
import { SoloDiscography } from '../../../../components/pages/solo-discography';
...
/**
 * Se prerenderizan las cuatro fichas en el build: son pocas, cambian poco y
 * así la primera visita no espera a ninguna llamada.
 */
export async function generateStaticParams() {
  try {
    // Las cuatro fichas por cada uno de los tres idiomas: doce paginas
    // estaticas. Son pocas y cambian poco, asi que compensa generarlas todas.
    const members = await getMembers({ locale: routing.defaultLocale });
    return routing.locales.flatMap((locale) =>
      members.map((member) => ({ locale, slug: member.slug })),
    );
  } catch {
    // Si la API no responde durante el build, las páginas se generan bajo
    // demanda en su lugar. Un build que falla por eso es un build frágil.
    return [];
  }
}
...
export default async function MemberPage({ params }: PageProps) {
  ...
  try {
    ...
    // Un 404 de la API es un 404 del sitio; cualquier otro fallo debe
    // propagarse para que lo recoja error.tsx y no se disfrace de "no existe".
    if (error instanceof ApiError && error.isNotFound) notFound();
    throw error;
  }

  const accent = member.colorAccent ?? 'var(--color-accent)';
  /* El color propio viaja como custom property; quien lo mezcla con la tinta
     del tema es `.bp-member-ink`. */
  ...
  return (
    <>
      ...
      <section className="relative isolate overflow-hidden">
        {/*
         * El color propio entra como un halo bajo el nombre, no como fondo
         * completo: a plena superficie, cuatro colores distintos romperían la
         * unidad del sitio; como halo, cada ficha se distingue sin dejar de
        ...
        <Container width="wide" className="pt-block pb-block">
          <div className="grid gap-10 lg:grid-cols-12 lg:gap-14">
            <div className="lg:col-span-5">
              {/* La foto principal de la ficha: es lo que domina la primera
                  pantalla, asi que va con `priority` y con un `sizes` que
                  refleja las cinco de doce columnas que ocupa. */}
            ...
            <div className="lg:col-span-7 lg:pt-8">
              ...
              {/* El color propio, mezclado con la tinta del tema: sin la
                  mezcla no llega a AA ni en claro ni en el modo BLINK. */}
        ...
        <Container width="wide" className="py-section">
          <Reveal>
            ...
            <div className="mt-block max-w-prose">
              {/* La biografía llega como texto plano; los párrafos son los
                  saltos de línea del propio dato, no marcado inventado. */}
              {member.bio
                .split('\n')
                .filter(Boolean)
                .map((paragraph, i) => (
      ...
      {/* --- Cronología personal, en vertical ----------------------------- */}
      ...
      {member.soloWorks.length > 0 ? (
        <Container width="wide" className="py-section">
          ...
          {/*
           * TODAS las canciones de cada obra, con un solo reproductor abierto
           * en toda la ficha. Ver `solo-discography.tsx`.
           */}
          <SoloDiscography
            ...
            labels={{
              ...
              /*
               * `.raw()`: estas cadenas llevan `{title}`, `{artists}` o
               * `{count}` y quien las sustituye es el componente, no
               * next-intl. Con la llamada normal lanzaría FORMATTING_ERROR.
               */
      ...
      {/* --- Curiosidades, en tarjetas volteables ------------------------- */}
...
function Fact({
  ...
  value: string | null;
  lang?: string;
  /** Texto ya formateado por idioma. `value` sigue siendo el valor legible por maquina. */
  display?: string | null;
}) {
  // Un dato que no está no se rellena con un guion: sencillamente no aparece.
