...
/**
 * ============================================================================
 * BATERIA DE INYECCION
 * ============================================================================
 * Un vector por bloque, con varias redacciones cada uno. Se comprueba que se
 * bloquea Y que dispara la regla esperada: si mañana un patron se "arregla" y
 * el intento se cuela por otra regla distinta, el test sigue en verde y nadie
 * se entera de que el patron propio dejo de funcionar.
 * ============================================================================
 */
...
describe('esquiva con caracteres invisibles', () => {
  /*
   * El truco mas barato para saltarse un filtro de texto: partir la palabra
   * con un caracter de ancho cero. Se ve identico y no casa con nada.
   */
  it('no se cuela una orden partida con un espacio de ancho cero', () => {
    ...
    expect(detectInjection(oculto).blocked).toBe(false); // sin limpiar, se cuela
    expect(attempt(oculto).blocked).toBe(true); // limpiando, no
  });
});
