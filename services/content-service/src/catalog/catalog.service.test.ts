...
/** Prisma simulado: solo lo que toca CatalogService. */
...
/*
 * Los DTOs reales exponen `verifiedFilter` como getter derivado de
 * `includeUnverified`. Los dobles lo replican para que el test ejercite la
 * misma traduccion a `where` que usa produccion, y no una version simplificada.
 */
  ...
  describe('curiosidades', () => {
    ...
    it('el desplazamiento aleatorio nunca deja menos elementos que el limite', async () => {
      ...
      for (const call of findMany.mock.calls) {
        // Con 10 filas y limite 4, el salto maximo valido es 6.
