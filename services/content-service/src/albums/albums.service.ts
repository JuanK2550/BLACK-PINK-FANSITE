...
export class AlbumsService {
  ...
  async list(query: AlbumsQueryDto): Promise<{ items: AlbumSummaryDto[]; pagination: PageMeta }> {
    ...
    /*
     * El total y la pagina se piden en la MISMA transaccion.
     * Con dos consultas sueltas, una insercion entre ambas produce un total
     * que no corresponde a los elementos devueltos y el cliente ve una pagina
     * fantasma al final del listado.
     */
  ...
  async findBySlug(
    ...
    const row = await this.prisma.album.findUnique({
      ...
      include: {
        translations: true,
        // El recuento cuenta las MISMAS pistas que se devuelven: si contase
        // todas, el detalle prometeria mas canciones de las que trae.
    ...
    // Un album sin contrastar es, para el publico, un album que no existe.
  ...
  /** Una pista suelta. La usa media-service para resolver un embed. */
  async findTrack(id: string, locale: Locale, includeUnverified: boolean) {
    ...
    // La cancion se oculta tambien si su album no esta contrastado: publicar la
    // pista de un album que no se publica seria una fuga por la puerta de atras.
