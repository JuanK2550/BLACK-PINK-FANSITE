...
/**
 * Punto unico por el que pasa todo antes y despues del modelo.
 *
 * Existe para que `ChatService` no tenga que acordarse del orden. El orden
 * importa: normalizar, luego detectar idioma (para poder negarse EN SU
 * IDIOMA), luego inyeccion, luego contenido. Detectar el idioma despues de
 * bloquear obligaria a responder en el idioma de la interfaz a alguien que
 * escribio en otro.
 */

export interface InputVerdict {
  /** Mensaje ya normalizado, listo para el modelo. */
  message: string;
  locale: Locale;
  localeDetected: boolean;
  /** Si viene, no se llama al modelo y se responde esto. */
...
export class SafetyService {
  ...
  /**
   * Revisa lo que ha escrito el modelo.
   *
   * Es un cinturon por encima del tirante: el prompt ya se lo prohibe y el
   * contexto solo trae contenido del sitio. Pero el modelo es de un tercero,
   * cambia de version sin avisar, y la unica forma de que una respuesta
   * inapropiada no llegue al visitante es mirarla.
   */
...
/**
 * Negativas escritas a mano, en los tres idiomas.
 *
 * NO se le piden al modelo: cuando se bloquea la entrada, el modelo ni se
 * llama. Y son AMABLES y CONCRETAS a proposito -dicen que no se puede y
 * ofrecen otra cosa- porque la mayoria de quien las va a ver no es un atacante
 * sino un fan que ha preguntado algo que aqui no se responde.
 */
