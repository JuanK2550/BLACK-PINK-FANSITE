...
export default async function PlaylistsPage({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  /*
   * El detalle de cada lista se pide EN PARALELO, no en serie. Son seis
   * llamadas; encadenadas sumarían seis latencias, y con `Promise.all` se
   * paga solo la más lenta. Todas están cacheadas en el gateway.
   */
  ...
  return (
    <>
      ...
      />
      {/*
       * Cada lista se declara como MusicPlaylist. `numTracks` sale del dato
       * real; no se declara `track` porque las canciones no son de este sitio
       * y enumerarlas aquí sugeriría que lo son.
       */}
      <JsonLd
        data={{
      ...
      <Container width="wide" className="pb-section">
        {/* El aviso de reproducción oficial, arriba y una sola vez: es una
            condición de toda la página, no de cada tarjeta. */}
              ...
              <article className="border-line flex h-full flex-col border-t pt-5">
                <div className="min-w-0">
                  ...
                  <p className="text-fg-subtle text-2xs mt-2" data-uppercase>
                    ...
                    {' · '}
                    {/* Se dice si la selección sale de un dato comprobable o
                        si es una elección del sitio. */}
