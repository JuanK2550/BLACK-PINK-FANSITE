...
/**
 * ============================================================================
 * NAVEGADOR DE DISCOGRAFÍA
 * ============================================================================
 * Filtra y ordena EN CLIENTE, sobre la lista completa que ya llegó del
 * servidor. Con nueve lanzamientos, ir a la red en cada cambio de filtro
 * añadiría medio segundo de espera a una operación que aquí es instantánea.
 * Si algún día la discografía creciera a cientos, esto pasaría al servidor.
 *
 * EL REORDENAMIENTO es lo que hace que el filtro se entienda: `layout` en cada
 * elemento hace que Framer Motion interpole la posición de los que
 * permanecen, en vez de que la rejilla salte a su nueva forma. El ojo puede
 * seguir un álbum de un sitio a otro, y eso convierte un cambio de estado en
 * una explicación de qué ha pasado.
 *
 * `mode="popLayout"` en AnimatePresence saca de flujo a los que se van, para
 * que los que quedan empiecen a moverse ya en vez de esperar a que termine la
 * salida.
 * ============================================================================
 */
export function DiscographyBrowser({ albums, locale }: DiscographyBrowserProps) {
  ...
  /*
   * Con movimiento reducido no hay interpolación de posición: los elementos
   * aparecen ya colocados. El reordenamiento animado de una rejilla entera es
   * mucho movimiento simultáneo, justo lo que hay que evitar.
   */
  ...
  return (
    <>
      ...
      {/* Recuento en vivo, anunciado a los lectores de pantalla. */}
            ...
            <AnimatePresence mode="popLayout" initial={false}>
              {visible.map((album) => (
                <motion.li
                  ...
                  // La salida es más rápida que la entrada: al filtrar, lo que
                  // se va ya no interesa y no debe hacerse esperar.
  ...
  return (
    ...
    >
      <AlbumCover
        ...
        zoom
        /* Dos columnas en movil, tres en tableta, cuatro en escritorio. */
  ...
  return (
    ...
    >
      <AlbumCover
        ...
        title={album.title}
        alt={alt}
        /* Miniatura de 56px: la variante pequena. */
        variant="thumb"
