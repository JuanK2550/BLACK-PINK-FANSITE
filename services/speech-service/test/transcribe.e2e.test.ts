...
/**
 * ============================================================================
 * EL STACK HTTP COMPLETO, SIN RED Y SIN CLAVE
 * ============================================================================
 * El proveedor es un doble. Es justo para esto para lo que existe la interfaz
 * `AsrProvider`: un test que necesita la API de Groq deja de ser reproducible
 * -se rompe el dia que se agota la cuota- y acaba desactivado, y un test
 * desactivado no protege nada.
 *
 * Lo que si es real: el pipe de validacion, el interceptor del envelope, el
 * filtro de errores, multer y el limitador. Que es donde estan los fallos.
 * ============================================================================
 */
...
/* Cabeceras minimas por formato: lo que mira `sniffAudio`. */
...
async function bootstrap(limit: number): Promise<INestApplication> {
  const moduleRef = await Test.createTestingModule({
    imports: [
      ...
      // ProviderModule entra para que el token exista en el grafo y se pueda
      // sustituir: `overrideProvider` reemplaza, no declara.
      ProviderModule,
      TranscribeModule,
    ],
  })
...
describe('POST /api/v1/transcribe', () => {
  ...
  beforeAll(async () => {
    // Limite alto para que las pruebas de validacion no se estorben entre si;
    // el limite de verdad tiene su propio bloque mas abajo.
  ...
  it('reenvia el audio con la extension deducida de los bytes, no con la del cliente', async () => {
    ...
    // El cliente miente en las dos: dice que es un mp3 llamado `../../etc/x`.
    ...
    // Ni la ruta ni la extension del cliente sobreviven.
  ...
  it('no manda pista de idioma si el cliente no la pide', async () => {
    ...
    // Forzar el idioma equivocado no da error: da texto plausible y falso.
  ...
  describe('validacion', () => {
    it('rechaza un fichero que no es audio aunque se anuncie como tal', async () => {
      ...
      // Y no se ha llegado a gastar una llamada al proveedor: se rechaza
    ...
    it('rechaza un audio mas largo que el maximo, descartando el texto', async () => {
      ...
      // Aceptarlo «porque ya esta hecho» convertiria el limite en una sugerencia.
...
describe('limite de transcripciones por IP', () => {
  ...
  it('corta al superarlo, con un codigo distinto del de la cuota del proveedor', async () => {
    ...
    // SPEECH_RATE_LIMIT ("has pedido demasiado tu") no es ASR_QUOTA ("se ha
    // agotado para todos"): el frontend dice cosas distintas.
