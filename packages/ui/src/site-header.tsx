...
export interface SiteHeaderProps {
  ...
  /** Los resultados vienen ya filtrados de la API: no se refiltran aqui. */
  searchRemote?: boolean;
  searchLoading?: boolean;
  onSearchQueryChange?: (query: string) => void;
  locale: Locale;
  /**
   * Prefijo de idioma para los enlaces internos (`/es`, `/en`, `/ko`).
   *
   * Se pasa como prop en lugar de importar el `Link` de next-intl: packages/ui
   * no debe depender del framework de rutas de la aplicacion. Asi el paquete
   * sigue siendo utilizable fuera de este proyecto y las pruebas no necesitan
   * montar un enrutador.
   */
  localePrefix?: string;
...
/** Retardo de intencion: evita que el menu se abra al cruzar el raton de paso. */
...
export function SiteHeader({
  ...
  useEffect(() => {
    // El estado solo cambia al cruzar el umbral, no en cada pixel: sin esto
    // el header provoca un render por fotograma durante todo el scroll.
    function onScroll() {
  ...
  /* --- Cierre del mega-menu al salir del header -------------------------- */
  ...
  return (
    <>
      ...
      >
        <div
          className={cn(
            // El cristal solo aparece cuando hay contenido detras que separar.
            // Sobre el hero el header es transparente y no compite con la portada.
          ...
          {activeItem?.panel ? (
            <motion.div
              ...
              exit={{
                ...
                filter: 'blur(4px)',
                // La salida siempre es mas rapida: al cerrar ya se ha decidido.
...
/** Filete que crece de izquierda a derecha bajo el elemento activo o en hover. */
  ...
  return (
    <span
      ...
      className={cn(
        ...
        // El hover solo aplica donde hay puntero fino: en tactil el :hover se
        // queda pegado despues de tocar.
        // En Tailwind v4 la variante hover ya lleva @media (hover: hover):
        // en tactil el subrayado no se queda pegado despues de tocar.
