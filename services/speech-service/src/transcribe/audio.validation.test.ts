...
/**
 * Cabeceras minimas, construidas a mano. No se leen ficheros de un directorio
 * de apoyo a proposito: lo que se prueba son los BYTES exactos que distinguen
 * un formato de otro, y en un binario de ejemplo esos bytes quedan escondidos
 * entre megabytes de audio. Aqui se ven.
 */
...
/** WAV con `fmt ` y `data`, para poder medir la duracion de verdad. */
function wav({ byteRate = 88200, dataBytes = 176400, junk = false } = {}): Buffer {
  ...
  if (junk) {
    // Un trozo intruso delante de `fmt `: los editores los meten, y por eso
    // no se puede asumir que `fmt ` este en el offset 12.
  ...
  fmt.writeUInt32LE(byteRate, 16); // bytes por segundo
  parts.push(fmt);
...
describe('sniffAudio: el formato sale de los bytes, no de lo que diga el cliente', () => {
  ...
  describe('rechaza lo que no es audio admitido', () => {
    it('un ejecutable de Windows, aunque se llame .webm y se anuncie audio/webm', () => {
      // El caso que motiva todo el modulo: extension y content-type los pone
      // el cliente y los dos se pueden mentir. Los bytes, no.
...
describe('probeDurationSec', () => {
  ...
  it('devuelve null para webm, ogg y mp3: se comprueban despues', () => {
    // No es un fallo. Ver la nota del modulo: la duracion de esos tres exige
    // recorrer el contenedor entero, y se resuelve con la que informa el
    // proveedor.
